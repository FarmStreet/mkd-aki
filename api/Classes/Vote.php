...
class Vote {
    ...
    public static function getList($vk_id) {
        ...
        $query = $db->prepare('SELECT users_votes.vk_id as vk_id, users_votes.vote_id as id, users_votes.is_agree as isAgree, users.name as name, votings.group_id as groupId 
...
LEFT JOIN users_groups ON (users_groups.group_id=votings.group_id) WHERE users_votes.vk_id=:vk_id OR users_groups.is_leader=1');
        ...
        $vote_list = $query->fetch(PDO::FETCH_ASSOC);

        return array_unique($vote_list);
    }
